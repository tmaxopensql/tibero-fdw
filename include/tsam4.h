/* ------------------------------ tsam4.h ----------------------------------- */
/*                                                                            */
/*                  Copyright (c) 2011 TmaxSoft Co., Ltd                      */
/*                        All Rights Reserved                                 */
/*                                                                            */
...
typedef struct {
    ...
    int   tsam_version;
    /* connection */
    int (*tsam_connect_entry)( tsam_connect_t *conn, int flags );
    int (*tsam_connect2_entry)( tsam_connect_t *conn, tsam_backup_t *back, int flags );
    int (*tsam_disconnect_entry)(int cd);
    /* transaction */
    int (*tsam_commit_entry)(int cd);
    int (*tsam_rollback_entry)(int cd);
    /* file management */
    ...
    /* file open session */
    int (*tsam_open_entry)(int cd, int fcnt,
                           tsam_file_t **file, tsam_report_t **report );
    int (*tsam_close_entry)(int fd);
    /* record access */
    int (*tsam_start_entry)(int fd, int aix_fd, void *ridfld, int keylen, int flags);
    int (*tsam_read_entry)(int fd, int aix_fd, void *ridfld, int keylen,
                           char *buf, int *buflen, int flags);
    int (*tsam_unlock_entry)(int fd, int aix_fd, int token);
    int (*tsam_write_entry)(int fd, int aix_fd, void *ridfld, int keylen,
                            char *buf, int buflen, int flags);
    int (*tsam_rewrite_entry)(int fd, int aix_fd, void *ridfld, int keylen,
                              char *buf, int buflen, int flags);
    int (*tsam_delete_entry)(int fd, int aix_fd, void *ridfld, int keylen, int flags);
    /* scan access */
    int (*tsam_start_br_entry)(int fd, int aix_fd, int reqid, void *ridfld,
                               int keylen, int flags);
    int (*tsam_reset_br_entry)(int fd, int aix_fd, int reqid, void *ridfld,
                               int keylen, int flags);
    int (*tsam_end_br_entry)(int fd, int aix_fd, int reqid);
    int (*tsam_read_next_entry)(int fd, int aix_fd, int reqid, void *ridfld,
                                int keylen, char *buf, int *buflen, int flags);
    int (*tsam_read_prev_entry)(int fd, int aix_fd, int reqid, void *ridfld,
                                int keylen, char *buf, int *buflen, int flags);
    /* build aix from base cluster */
    int (*tsam_bldindex_entry)(int fd, int basefd);
    /* XA connect for multiple RM */
    int (*tsam_xa_connect_entry)(char *xa_conn_id, int flags);
} tsam_switch_t;
...
#define tsam_connect(conn, flags) (*(tsam_sw.tsam_connect_entry))(conn, flags)
#define tsam_connect2(conn, back, flags) (*(tsam_sw.tsam_connect2_entry))(conn, back, flags)
#define tsam_xa_connect(conn_id, flags) (*(tsam_sw.tsam_xa_connect_entry))(conn_id, flags)
 ...
 *
 *   int tsam_commit(int txid);
 *   int tsam_rollback(int txid);
 **/
#if 0
...
#define tsam_commit(cd) (*(tsam_sw.tsam_commit_entry))(cd)
#define tsam_rollback(cd) (*(tsam_sw.tsam_rollback_entry))(cd)
 ...
 * XXX Original TSAM API:
 *
 *   int tsam_open(int cd, int fcnt, tsam_file_t **file, tsam_report_t **report);
 *   int tsam_close(int fd);
 *   int tsam_bldindex(int fd, int basefd);
 **/

#define tsam_open(cd, fcnt, file, report) \
    (*(tsam_sw.tsam_open_entry))(cd, fcnt, file, report)
 ...
 *   int tsam_read(int fd, int aix_fdvoid *ridfld, int keylen, char *buf, int *buflen, int flags);
 *   int tsam_unlock(int fd, int aix_fd, int token);
 *
 *   int tsam_write(int fd, int aix_fd, void *ridfld, int keylen, char *buf, int buflen, int flags);
 *   int tsam_rewrite(int fd, int aix_fd, void *ridfld, int keylen, char *buf, int buflen, int flags);
 *   int tsam_delete(int fd, int aix_fd, void *ridfld, int keylen, int flags);
 **/

#define tsam_start(fd, aix_fd, ridfld, keylen, flags) \
    (*(tsam_sw.tsam_start_entry))(fd, aix_fd, ridfld, keylen, flags)
#define tsam_read(fd, aix_fd, ridfld, keylen, buf, buflen, flags) \
    (*(tsam_sw.tsam_read_entry))(fd, aix_fd, ridfld, keylen, buf, buflen, flags)
#define tsam_unlock(fd, aix_fd, token) (*(tsam_sw.tsam_unlock_entry))(fd, aix_fd, token)
#define tsam_write(fd, aix_fd, ridfld, keylen, buf, buflen, flags) \
    (*(tsam_sw.tsam_write_entry))(fd, aix_fd, ridfld, keylen, buf, buflen, flags)
#define tsam_rewrite(fd, aix_fd, ridfld, keylen, buf, buflen, flags) \
    (*(tsam_sw.tsam_rewrite_entry))(fd, aix_fd, ridfld, keylen, buf, buflen, flags)
#define tsam_delete(fd, aix_fd, ridfld, keylen, flags) \
    (*(tsam_sw.tsam_delete_entry))(fd, aix_fd, ridfld, keylen, flags)
 ...
 * XXX Original TSAM API:
 *
 *   int tsam_start_br(int fd, int aix_fd, int reqid, void *ridfld, int keylen, int flags);
 *   int tsam_reset_br(int fd, int aix_fd, int reqid, void *ridfld, int keylen, int flags);
 *   int tsam_end_br(int fd, int aix_fd, int reqid);
 *
 *   int tsam_read_next(int fd, int aix_fd, int reqid, void *ridfld, int keylen, char *buf, int *buflen, int flags);
 *   int tsam_read_prev(int fd, int aix_fd, int reqid, void *ridfld, int keylen, char *buf, int *buflen, int flags);
 **/

#define tsam_start_br(fd, aix_fd, reqid, ridfld, keylen, flags) \
    (*(tsam_sw.tsam_start_br_entry))(fd, aix_fd, reqid, ridfld, keylen, flags)
#define tsam_reset_br(fd, aix_fd, reqid, ridfld, keylen, flags) \
    (*(tsam_sw.tsam_reset_br_entry))(fd, aix_fd, reqid, ridfld, keylen, flags)
#define tsam_end_br(fd, aix_fd, reqid) (*(tsam_sw.tsam_end_br_entry))(fd, aix_fd, reqid)

#define tsam_read_next(fd, aix_fd, reqid, ridfld, keylen, buf, buflen, flags) \
    (*(tsam_sw.tsam_read_next_entry))(fd, aix_fd, reqid, ridfld, keylen, \
                                      buf, buflen, flags)
#define tsam_read_prev(fd, aix_fd, reqid, ridfld, keylen, buf, buflen, flags) \
    (*(tsam_sw.tsam_read_prev_entry))(fd, aix_fd, reqid, ridfld, keylen, \
                                      buf, buflen, flags)
